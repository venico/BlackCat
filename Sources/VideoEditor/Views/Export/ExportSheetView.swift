...
struct ExportSheetView: View {
    ...
    /// 默认用时间轴上**第一个视频**的名字（去掉扩展名）；
    /// 没有视频再退回项目名，项目也没命名就用时间戳
    private func defaultFilename() -> String {
        if let first = project.videoTracks.flatMap(\.clips)
            .min(by: { $0.startTime < $1.startTime }) {
            let base = (first.name as NSString).deletingPathExtension
...
struct ExportInput {
    ...
    let effectTracks:   [Track<EffectClip>]
...
actor TimelineExporter {
    ...
    func export(_ input: ExportInput,
                ...
                if includeVideo && track.isVisible,
                   let vAsset = try? await exEffAsset.loadTracks(withMediaType: .video).first {
                    ...
                    if clip.id == firstVideoClipID {
                        ...
                        sourceVideoSize = OverlayRenderer.orientedNaturalSize(
        ...
        if includeVideo {
            ...
            let subRenderInfo = OverlayRenderer.SubtitleRenderInfo(
            ...
            // 复合片段**内部**的字幕/文字/图形/图片同样要逐帧烧录。
            // 上面那几个 visibleXxxClips 收的都是顶层轨道，不含复合片段里的内容——
            // 漏掉它的话，一个「只有复合片段、顶层没有任何 overlay」的项目会被判成
            // 不需要逐帧处理，直接走下面的快速路径，复合片段里的字幕一个字都进不了成片。
            // flattened() 是为了连嵌套复合片段里的也算上
            let compoundHasOverlays = input.compoundTracks
                .filter(\.isVisible)
                .flatMap(\.clips)
                .contains { clip in
            ...
            try await writerExport(
                ...
                effectTracks: input.effectTracks,
                                        ...
                                        for ref in layersBottomUp {
                                            ...
                                            case .subtitle(_):
                                                if !subtitleRendered && hasSubtitles {
                                                    ...
                                                    if subKey != cachedSubKey {
                                                        cachedSubOverlay = OverlayRenderer.renderSubtitleOverlay(atTime: targetTime, info: subtitleInfo)
                                                        cachedSubKey = subKey
                                                    }
                                            ...
                                            case .text(let trackID):
                                                if let clips = textClipsByTrack[trackID],
                                                   let overlay = OverlayRenderer.renderTextOverlay(
                                            ...
                                            case .shape(let trackID):
                                                if let clips = shapeClipsByTrack[trackID],
                                                   let overlay = OverlayRenderer.renderShapeOverlay(
                                                       atTime: targetTime, clips: clips,
                                            ...
                                            case .effect(let trackID):
                                                if let track = effectTracks.first(where: { $0.id == trackID }),
                                                   track.isVisible {
                                                    for clip in track.clips
                                                    where clip.startTime <= targetTime && clip.endTime > targetTime {
                                                        image = EffectEngine.apply(clip, to: image,
                                                                                   renderSize: renderSize)
                                                            .cropped(to: CGRect(origin: .zero, size: renderSize))
                                                    }
                                                }
                                            ...
                                            case .compound(let trackID):
                                                image = OverlayRenderer.composeCompoundOverlays(
                                                    trackID: trackID, tracks: compoundTracks,
    ...
    /// 把一条复合轨道在 `targetTime` 时刻的 overlay 子内容（图片/字幕/文字/图形）
    /// 合成到 image 上，返回合成后的结果。轨道上这一刻没有复合片段时原样返回。
    ///
    /// 两个调用点共用同一份实现：`overlayTrackOrder` 里登记过的按顺序合成，
    /// 没登记的（含视频的复合片段只进 videoSectionOrder）在所有 overlay 之前合成。
    /// 抽出来是因为这两处必须完全一致——写两遍就是漏一处的开始
    ...
    private nonisolated func drawSubtitlesOnPixelBuffer(
        _ pixelBuffer: CVPixelBuffer, atTime time: Double, info: OverlayRenderer.SubtitleRenderInfo
    ) {
        // 找出当前时间活跃的字幕
        ...
        for (track, style) in info.tracks {
            if let clip = track.clips.first(where: { $0.startTime <= time && $0.endTime > time }) {
                let text = style.mergeLineBreaks ? OverlayRenderer.mergeBreaks(clip.text) : clip.text
