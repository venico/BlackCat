...
**采纳**：本地 Real-ESRGAN，**x2/x4 各用一个官方原生模型**（`RealESRGAN_x2plus` + `RealESRGAN_x4plus`），而不是只维护 x4 模型再降采样出 2 倍——原生模型效果更好。代价是两个模型都要单独转换验证、单独下载（各约 64MB）。不做动漫版变体（`x4plus_anime_6B`）——黑猫剪辑是通用剪辑软件，先把这两个跑通，不做更多变体。
...
2. **逐帧推理**：CoreML 跑对应倍数的 Real-ESRGAN 模型（用户选 x2 就跑 `x2plus`，选 x4 就跑 `x4plus`）
...
清晰度提升
┌─────────────────────────────────────┐
│ Real-ESRGAN x2        [已安装/下载/进度条] │
│ 视频超分辨率模型 2倍放大（CoreML），约 XX MB │
├─────────────────────────────────────┤
│ Real-ESRGAN x4        [已安装/下载/进度条] │
│ 视频超分辨率模型 4倍放大（CoreML），约 XX MB │
└─────────────────────────────────────┘
...
1. **RRDBNet → CoreML 转换是否真的顺畅**（未验证，方案的最大不确定性，x2plus/x4plus 两个模型都要各自验证一遍）
2. **CoreML 在 Apple Silicon 上跑 Real-ESRGAN 单帧推理的实际耗时**（未实测，x2/x4 分别测，影响"耗时提示"的具体数值和整体可用性判断）
